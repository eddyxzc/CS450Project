...
#define LIST  4
#define BACK  5
//#define NONOHUP 6// added new cmd
#define MAXARGS 10
...
runcmd(struct cmd *cmd)
{
  ...
  case EXEC:
    ...
    // then we start a new process, run the program in the child process and return the parent process
    // here, the parent process won't wait child process finish, this make the program run in the back
    ...
    if (*s==0)//this is a nonnohup cmd
    {
      ...
      for(;argc<MAXARGS-1;argc++){
        if(ecmd->argv[argc]==0)
        {
          break;//no more arg, cause it point to nothing;
        }else{
      ...
      {
        close(1);
        open("nonohup.output",O_WRONLY|O_CREATE);//redirect output for standar output to"nonohup.output"
...
gettoken(char **ps, char *es, char **q, char **eq)
{
  ...
  default:
    ret = 'a';
    while(s < es && !strchr(whitespace, *s) && !strchr(symbols, *s))//stop at whitespace or symbol
      s++;
    break;
  }
  if(eq)
  ...
  while(s < es && strchr(whitespace, *s))//skip whitespace
    s++;
  *ps = s;//ps move to next token,like offset
  return ret;
}
...
parseexec(char **ps, char *es)
{
  ...
  while(!peek(ps, es, "|)&;")){
    if((tok=gettoken(ps, es, &q, &eq)) == 0)
      break;
    if(tok != 'a')// cmd has some wired symble
      panic("syntax");
    cmd->argv[argc] = q;
    cmd->eargv[argc] = eq;
    argc++;
